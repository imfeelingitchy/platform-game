...
while run:
    ...
    draw_text('Sign Up', 40, 40 + math.sin(fc / 20) * 10, 100, greentheme[1])
    draw_text('Username:', 40, 190, 50, greentheme[1])
    draw_text('Password:', 40, 270, 50, greentheme[1])
    draw_text('Confirm Password:', 40, 350, 50, greentheme[1])
    entry((530, 190), 0, type_ = '_-')
    entry((530, 270), 1, type_ = 2)
    entry((530, 350), 2, type_ = 2)
    button(250, 80, browntheme[0], browntheme[1], ('Sign Up', 45, 450, 50, browntheme[3], 255), (28, 7), 1)
    button(250, 80, browntheme[0], browntheme[1], ('Back', 345, 450, 50, browntheme[3], 255), (55, 7), 2)
    if invalid == -1:
        draw_text('Username must be at least 5 characters long.', 40, 550, 40, redtheme[1])
    elif invalid == 1:
        draw_text('This username is already taken.', 40, 550, 40, redtheme[1])
    elif invalid == 2:
        draw_text('Given passwords did not match.', 40, 550, 40, redtheme[1])
    elif invalid == 3:
        draw_text('Password must be at least 10 characters long.', 40, 550, 40, redtheme[1])
    elif invalid == 4:
        draw_text('Password must start with an alphabet.', 40, 550, 40, redtheme[1])
    ...
    if button_clicked == 2:
        ...
        if len(text[0][1]) < 5:
            invalid = -1
        elif text[0][1] in temp2:
            invalid = 1
        elif text[1][1] != text[2][1]:
            invalid = 2
        elif len(text[1][1]) < 10:
            invalid = 3
        elif not text[1][1][0].isalpha():
            invalid = 4
        else:
            cur.execute('insert into users values (%s, %s)', (text[0][1], text[1][1]))
            con.commit()
            comm.rval = 4
            comm.uname = text[0][1]
            run = False
